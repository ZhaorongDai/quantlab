...
- `as_of(permno, day)` is the strict, single-value question. A missing or
  unreadable sidecar RAISES: the caller asked which name a specific security
  wore on a specific day, and "I could not read the file" is not an answer that
  may be silently rounded to `None`.
- `label(permnos, day)` is the DISPLAY entry point, and it never raises. The
  six human-visible points that call it -- the forced-liquidation log and
  `liquidations.json`, the model's missing/extra symbol lists,
  `UniverseMask.report()`'s missing-member list, `browse_zarr`'s refusal and
  the `--symbols` CLI help -- are all trying to make an EXISTING message
  readable. Breaking a backtest because an audit sidecar is absent would make
  the readability layer more fragile than the thing it annotates (T-03.11-30),
  so an unreachable sidecar degrades to the digits, which is exactly what those
  messages printed before this sidecar existed.
...
class CrspTickerLookup:
    ...
    def as_of(self, permno: int, day: date) -> str | None:
        ...
        """
        spans = self.payload.get("intervals", {}).get(str(int(permno)))
        if not spans:
            return None
        as_of = str(day)[:10]
        for span in spans:
    ...
    def label(self, permnos: Sequence, day: date) -> list[str]:
        ...
        **Never raises.** An unknown PERMNO falls back to its own digits, and
        so does every PERMNO when the sidecar is missing or corrupt -- which is
        precisely the output these messages produced before the sidecar
        existed, so a panel with no sidecar (a Tiingo or Alpaca store, or a
        CRSP store built before 03.11-09) reads exactly as it did. `as_of`
        keeps the strict behaviour for callers who want the refusal.
        """
        try:
            intervals = self.payload.get("intervals", {})
