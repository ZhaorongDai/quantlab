...
- `scripts/wrds/index.py` — one index's point-in-time CRSP daily bars plus its membership panel (`--index sp500|nasdaq100 --start [--end] [--refresh] [--download-dir] [--zarr-dir]`).
- `scripts/wrds/market.py` — the CRSP daily market plus its listing panel (`--start [--end] [--security-filter] [--refresh] [--download-dir] [--zarr-dir]`); stores are `wrds_crsp_market_*`.
- `scripts/wrds/etf.py` — one store per ETF by PERMNO (`--etf spy,qqq,name=PERMNO --start [--end] [--refresh] [--download-dir] [--zarr-dir]`).
- `scripts/wrds/nbbo.py` — TAQ NBBO quotes resampled into a bar panel (`--symbols|--index, --start [--end] [--interval] [--session HH:MM-HH:MM] [--refresh] [--download-dir] [--zarr-dir]`).
- Every script always converts to Zarr, clips `--end` (default today) to the product's last date, checks entitlement before downloading and closes the shared WRDS session in a `finally`. `--download-dir` and `--zarr-dir` both default to the current directory: raw files go to `<download-dir>/wrds/` (watermarks in `_watermarks/wrds/`, CRSP reference tables in `_reference/`), the stores straight into `<zarr-dir>/`; the scripts do not use the library's data root. `scripts/` is not on the pytest `pythonpath`: `scripts/wrds/` must never be importable, because it would shadow the `wrds` PyPI package. Tiingo, Alpaca and Binance have library interfaces only.
