...
| `FactorAnalyzer` / `FactorAnalysis` / `FactorReportFigure` | alphalens-style factor report behind `Factor.analyze(factor_names, frets, output_dir, quantiles)`: bar-spacing check, inner join of factor and forward-return panels, IC/quantile-return/turnover metrics, one composite matplotlib figure per pair, optional output directory with tidy CSVs, `summary.json` and a rebuildable `config.json`. `quantlab/base/factor.py` imports it lazily inside `analyze()` so the base layer never imports the analysis layer at import time | `quantlab/analysis/factor_report.py` |
...
- Resample (added 2026-09-25): `resample(freq, how)` on any `BaseDataset` or `Factor` returns a memory-independent copy whose config carries `resample_freq`/`resample_how` and whose panel is aggregated onto coarser bars (`quantlab/utils/resample.py` holds the shared checks and bucket labels; `DataBackend.resample(labels, how)` does the grouping). The dataset owns how bars are cut (`_resample_labels`, UTC-clock floor by default, NYSE sessions in `NbboPanelDataset`); a factor delegates to its dataset. A resampled object is a view: it refuses `from_raw_data*`/`update`/streaming, and `save()` goes to `store_path` (`<stem>_resample_<freq>.zarr` beside the source), which `read()` prefers when it exists (`docs/adr/0002-resampled-store-is-a-sibling-cache.md`).
...
- `scripts/wrds/index.py` — one index's point-in-time CRSP daily bars plus its membership panel (`--index sp500|nasdaq100 --start [--end] [--refresh] [--max-workers] [--download-dir] [--zarr-dir]`).
- `scripts/wrds/market.py` — the CRSP daily market plus its listing panel (`--start [--end] [--security-filter] [--refresh] [--max-workers] [--download-dir] [--zarr-dir]`); stores are `wrds_crsp_market_*`.
- `scripts/wrds/etf.py` — one store per ETF by PERMNO (`--etf spy,qqq,name=PERMNO --start [--end] [--refresh] [--max-workers] [--download-dir] [--zarr-dir]`).
- `scripts/wrds/nbbo.py` — TAQ NBBO quotes resampled into a bar panel (`--symbols|--index, --start [--end] [--interval] [--session HH:MM-HH:MM] [--refresh] [--max-workers] [--download-dir] [--zarr-dir]`).
- Every script always converts to Zarr, clips `--end` (default today) to the product's last date, checks entitlement before downloading and closes the shared WRDS session in a `finally`. `--download-dir` and `--zarr-dir` both default to the current directory: raw files go to `<download-dir>/wrds/` (watermarks in `_watermarks/wrds/`, CRSP reference tables in `_reference/`), the stores straight into `<zarr-dir>/`; the scripts do not use the library's data root. `--max-workers` (default 4, at most 6) downloads batches in parallel: `WrdsSession` is a connection pool capped at `MAX_CONNECTIONS = 6`, opened on demand, never reconnected after a failure. `scripts/` is not on the pytest `pythonpath`: `scripts/wrds/` must never be importable, because it would shadow the `wrds` PyPI package. Tiingo, Alpaca and Binance have library interfaces only.
- `examples/` — runnable walkthroughs (`quickstart.py`, `build_panel.py`, `train_model.py`, `backtest.py`, `inspect_data_sources.py`, `wrds_us_equity/`). The WRDS directory is one SELF-CONTAINED file per model (`xgb.py`, `xgb_td.py`, `realmlp.py`) plus `factor_analysis.py`: each imports only quantlab, carries its own `DATA_ROOT` constant and flat `Settings` dataclass, and duplicates the shared steps on purpose (the user wants minimal runnable examples that can be copied out alone; do not factor a `common.py` back out). `pipeline.py` was split this way on 2026-09-26; walk-forward CV was dropped from the examples then.
