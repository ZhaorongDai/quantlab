...
```
   ...
   Parallel path (Binance, D-03/D-04 — config retrofit only, no new fetch logic):
          ...
          may live outside data/{market}/{frequency}/... via --raw-data-dir override)
                               │
                               ▼
          data/downloads/crypto_spot/1d/spot/monthly/klines/{SYMBOL}/...
          (or a --raw-data-dir-overridden external path, e.g. ~/Downloads/spot/monthly/klines/)
                               │ read by
                               ▼
...
- **Migrating/copying a user's real, already-downloaded raw data files to fit a new path convention:** unnecessary filesystem mutation for a multi-GB local dataset when a config-level override (`--raw-data-dir`) achieves the same result with zero risk of data loss or duplication (see Open Questions, resolved below).
...
1. **Where should the retrofitted `spot_kline_config()`'s `raw_data_dir_path` actually point?** *(RESOLVED: 2026-09-05, during 02-05-PLAN.md revision)*
   ...
   - **Resolution:** Neither — `ingest_binance_spot.py` (02-05) exposes a `--raw-data-dir` CLI override applied to the constructed `DatasetConfig` at run time. `spot_kline_config()`'s default still derives from the `data/{market}/{frequency}/...` convention (keeping the config schema itself uniform per D-02), but the CLI entry point lets the user point a specific run directly at wherever their CSVs already live, with zero filesystem mutation and zero change to any file besides `ingest_binance_spot.py`. This is the lighter-weight fix, consistent with D-04's "retrofit only, no new ingestion/migration logic" scope boundary.
...
2. **Does the Tiingo EOD API actually return adjusted/corporate-action fields by default?**
   ...
   - **Mitigated by design, not resolved via live call:** `TiingoAcquisition` (02-04) always passes an explicit `columns=` parameter (see Code Examples/Pitfall 3), which sidesteps the ambiguity entirely regardless of what Tiingo's undocumented default actually is. 02-VALIDATION.md's Manual-Only Verifications table has been updated to reflect that no live-API smoke test is required for this phase — see that document for the rationale.
...
| External Binance raw CSVs (`~/Downloads/spot/monthly/klines/...`) | `SpotKlineDataset` retrofit (D-04) | ✓ present on this machine, but outside the project's `data/` root | — | Resolved via `--raw-data-dir` CLI override in `ingest_binance_spot.py` (02-05) — see Open Question 1 (RESOLVED) |
...
| DATA-03 | Adding a new market/frequency requires only a new `Dataset` subclass + config, no factor/model/backtest changes | design/code review (per ROADMAP Success Criterion 4, explicitly not proven by building a third source) + one automated proof: a minimal test-only `FakeDataset` subclass + config exercising the full `from_raw_data()`→`save()`→`read()` lifecycle without touching non-`dataset/config` files | `uv run pytest tests/test_extensibility_contract.py -x` (automated proof) + manual code review checklist (02-06's `checkpoint:human-verify` task) | ❌ Wave 0 |
...
| Unbounded/unvalidated raw file paths (`raw_data_dir_path`) pointing outside the intended data root | Tampering / Information Disclosure (path traversal-adjacent) | Low severity here since all paths are locally-configured (not user/network-supplied at runtime), but the new `stock_kline_config()` factory should still derive paths via the same `_data_root()` helper as existing factories rather than accepting arbitrary absolute strings from an untrusted source; the `--raw-data-dir` override (02-05) is a local, single-user CLI argument, not a network-supplied value, so the same low-severity acceptance applies |
...
- Pitfalls: HIGH for the three documented pitfalls (all empirically reproduced or directly evidenced from real on-disk file layouts) — MEDIUM for the Tiingo default-columns question specifically (A1), which remains genuinely unresolved without a live API call but is fully mitigated by design (explicit `columns=`)
