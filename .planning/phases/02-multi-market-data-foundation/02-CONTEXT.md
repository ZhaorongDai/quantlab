...
### US equity universe coverage + classification (post-completion addition)
- **D-12:** After Phase 2's original 7 plans landed, the user asked for US-equity ingestion to cover the **full Nasdaq-listed universe** plus the **S&P 500 constituent list**, with each symbol tagged by category. This is genuinely new scope beyond the original DATA-01 wording — the user explicitly chose to add it as a Phase 2 supplementary plan rather than defer it.
- **D-12a (critical correctness correction — user's explicit words):** "这里的成分股不能泄露未来数据，要根据历史上真实的不同成分股来实时调整。我要获取的美股数据也要包括退市的股票" — the classification must be **survivorship-bias-free and look-ahead-free**: a *point-in-time* record of which symbols were actually S&P 500 constituents on any given historical date (not a static "current members" snapshot retroactively applied to history), and the equity universe itself must include **historically delisted stocks**, not just currently-listed ones.
- **Data sources (verified reachable, no auth required, by the orchestrator directly via curl before spawning research):**
  - Tiingo's own ticker directory: `https://apimedia.tiingo.com/docs/tiingo/daily/supported_tickers.zip` → `supported_tickers.csv` (`ticker,exchange,assetType,priceCurrency,startDate,endDate`, ~108k rows across all markets Tiingo tracks). A ticker's `endDate` well before today indicates it stopped trading (delisted) — this is the leading candidate for a full-market-including-delisted universe source, since it's Tiingo's own queryable universe with explicit valid date ranges per symbol, superseding the originally-considered `nasdaqlisted.txt` (which only lists currently-active tickers, no delisted history).
  - Wikipedia's "Historical components of the S&P 500" page has a dated wikitable of every S&P 500 addition/removal event since 1963 (each row: effective date, ticker/security added, ticker/security removed) — a candidate source for reconstructing point-in-time membership intervals.
  - S&P 500 CURRENT constituents: `https://raw.githubusercontent.com/datasets/s-and-p-500-companies/main/data/constituents.csv` — usable as a known-correct anchor at today's date to roll backward through the Wikipedia change log.
- **Open question sent to research (D-12b):** the user asked whether Tiingo itself might offer point-in-time index-constituent data natively (more robust for ongoing daily refresh than scraping Wikipedia, which has no formal API/update guarantee) — `02-08-RESEARCH.md` investigates this before the plan is finalized. If Tiingo has no such native data, the plan must include a robustness strategy for the Wikipedia-scrape path (e.g., fail gracefully to last-known-good data rather than crashing on scrape/parse failure, since this needs to support daily refresh).
- **Classification model (subject to research confirmation):** a point-in-time interval table — `(symbol, category, start_date, end_date)` — not a single static tag, queried as "what was symbol X's category on date D" or "which symbols were category C on date D," resolved dynamically rather than baked in as a fixed label.
- **Architecture (per [[feedback-encapsulate-new-components]]):** must be a properly encapsulated, class-based component — not a flat script. Exact placement (new `acquisition/universe.py` class, relationship to the existing OHLCV-shaped `Acquisition(ABC)`, storage format for the interval table) is being determined by `02-08-RESEARCH.md` against the existing codebase architecture.
- **Wiring:** `ingest_tiingo.py`/`stock_kline_config()` should be able to resolve a symbol list from this universe by category **as of a given date** (e.g. "give me the S&P 500 membership as of 2015-06-01"), not just "give me today's members" — critical for building a correct historical backtesting universe.
