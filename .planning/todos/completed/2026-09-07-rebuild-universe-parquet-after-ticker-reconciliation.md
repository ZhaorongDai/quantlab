...
Rebuilt with `uv run python refresh_us_equity_universe.py` (repo ROOT, not
`scripts/` — the path in "How to close this" above was wrong) and, contrary to
that section, **no API key was needed**: the script's own `--help` says
"Requires no API key" because it pulls Tiingo's public `supported_tickers.csv`.

Measured after the rebuild:

- rows 25,709 -> 25,696 (-13); unique symbols 14,602 -> 14,593 (-9) — exactly
  the 13 malformed row-entries / 9 distinct literals the plan measured.
- The closing check returns `unfetchable symbols remaining: []`. It validates
  against `enums.data.TRADEABLE_TICKER_PATTERN`, which is the SAME object
  `base/acquisition.py:_TICKER_PATTERN` is bound to, so an empty list means the
  fetch-time guard cannot refuse any symbol now in the table.
- `ingest_us_equity.py --start-date 2025-09-07 --end-date 2026-09-06 --dry-run`
  completes without halting: 8,159 symbols resolved for the window, 0.18 GiB
  dense, 2 year-chunks, largest 0.13 GiB against a 4.00 GiB per-chunk budget.

Still NOT verified (unchanged, and not claimed): a live full-market `download()`
run, which does need `TIINGO_API_KEY`. The pre-flight is where this todo's
failure lived, and the pre-flight now passes.

The carried-forward `NSPR-WSB` finding above stands: that one microcap warrant
series is gone from the roster, deliberately.
