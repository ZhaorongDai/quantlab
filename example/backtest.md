...
| `whole` | 引擎的整段统计：vectorbt `Portfolio.stats()` 全套指标（去掉 `benchmark_return`），加 `turnover`，再加 `order_count`（整段的成交笔数）。其中交易类指标是**持仓级**口径，见下文「交易统计的口径」 |
...
**切片块的两个交易计数与整段 `whole` 是同一个口径（持仓级），所以两边对得上账。** `in_sample.closed_trade_count`
与 `out_of_sample.closed_trade_count` 相加等于 `whole` 的 `Total Closed Trades`，段末未平仓数同理。

---

## 交易统计的口径

**交易指标只有一套口径：持仓级（D-02）。** 一笔交易 = 一个标的从建仓到清空，中途减仓不单独算一笔。

为什么不用 vectorbt 默认的 exit trades 口径：它把每一次减仓都记成一笔独立的**已平仓交易**，而等权调仓下
只有**赢家**才需要被削回目标权重，于是胜率被系统性抬高。两次独立的真实运行实测都偏高约 6.5 个百分点：
lot 级胜率 57.27%，持仓级分别是 50.74%（2025 单年）与 50.81%（2020–2026 六年）。亏损一侧几乎不受影响
（-6.28% vs -6.32%），正是「只有赢家会被削」所预测的形状。所以顶层直接报持仓级，不再并列报 lot 级，
也不再有嵌套的 `positions` 子字典。

整段「到底成交了多少次」由 `whole` 的 `order_count` 回答——它数的是订单记录的条数，也就是真实成交笔数。
`Total Fees Paid` 与 `turnover` 只回答成本、不回答次数，所以这个计数是必要的补充，而不是可选的附加。
...
于是算出一份和 exit trades 逐字节相同、看起来却没问题的假结果。也不去改 vectorbt 的全局设置映射——那会波及
同进程里的每一个组合对象。
...
- **交易统计是持仓级口径（见上文「交易统计的口径」）**，`notes` 里另有一条写明这一点，并说明 `order_count` 数的是真实成交笔数；
...
向上三角是它的**最低点**（那段回撤里最深的那个 bar），向下三角是它**修复**的那个 bar，所以两者之间量的是
**从底部回到本金花了多久**，而不是这段回撤是从哪个 bar 开始的。（2026-09-15 的 quick 260915-v6i 原本标的是起点，
用户看过之后明确要求改成最低点。）
三角之间的长度一律按**交易日（bar 数）**计，不是日历天：时间轴跨过的日历天数比这个数大，拿轴去量会读出一个不一样的数。
挑中的是**最深**的那一条回撤记录（按 `valley_val / peak_val - 1` 选），而**不是**持续最久的那一条。

**这个长度不是 `Max Drawdown Duration`，而且通常比它小**，有两条彼此独立的原因：
一是那个指标量的是**最久**的那一次回撤，和这里选中的**最深**那次经常不是同一段（实测过的一段净值：三条记录深度
`[-36.4%, -5.2%, -5.9%]`、时长 `[1, 5, 1]` 个 bar，最深的那条只持续 1 个 bar，最久的那条有 5 个）；
二是就算碰巧是同一段，那个指标从回撤**开始**算起，而这里从**最低点**算起。所以页面上这两个数对不上是正常的，`notes` 里也写了这件事。
最深那次回撤直到最后一个 bar 都没有修复时，页首那一行和三角的悬浮文字都会写明「没有修复」，不会说成已经结束；
这种情况下最低点可能就是最后一个 bar，那么长度是 0 个 bar，这也是对的。
