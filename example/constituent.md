...
{'in_window_members': 3, 'missing_count': 1, 'missing_symbols': ['CCC'], 'missing_labels': ['CCC']}
...
3. **覆盖缺口被点名**：`CCC` 是成分股但价格面板没有它，`report()` 给出完整列表（**永不截断、永不采样**——一个被截断的列表比没有列表更糟，因为它看起来像个完整答案）。`DDD` 在价格面板里但不是成分股，一声不吭地丢掉——这是那两个刻意的不对称。
   报告有**两份**缺失清单，等长同序：`missing_symbols` 是**轴标签本身**（机器那一半——要把它们 `.sel()` 出来需要身份），`missing_labels` 是**给人读的名字**。这个库旁边没有 ticker 旁车，所以两者相同；CRSP 面板上前者是一串 int64 PERMNO，后者是窗口最后一天的 ticker 拼写（读者手里正在看的那个），查不到名字的条目保留数字而不是从清单里掉出去。
