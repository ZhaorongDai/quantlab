...
| Script | What it shows | Guide |
|--------|---------------|-------|
| [`wrds_us_equity/`](wrds_us_equity/) | The full pipeline on CRSP daily data for the point-in-time S&P 500 or Nasdaq-100, one file per model (`xgb.py`, `xgb_td.py`, `realmlp.py`: Alpha101 + Alpha158 factors, a forward-return label, the model trained once or walk-forward, a TopN backtest, Weights & Biases logging) plus `factor_analysis.py`, an alphalens-style report on every alpha column. Needs a WRDS account and a converted CRSP store; the data root is a constant in `common.py` and every setting is a dataclass | [README](wrds_us_equity/README.md), [WRDS](../docs/wrds_crsp.md) |
