"""XGBoost pipeline on the point-in-time Nasdaq-100, from WRDS CRSP daily bars.

CRSP daily bars -> Alpha101 + Alpha158 factors -> open-to-open forward-return
label -> ``XGBoostRegressor`` (``xgb.train``, native early stopping) -> TopN
cross-sectional backtest against buy-and-hold QQQ, logged to Weights & Biases.

Every setting is a constant or a quantlab config object at the top of the
file; edit them and run ``uv run python examples/wrds_us_equity/nasdaq100_xgb.py``
...
def backtest(checkpoint: Path):
    ...
    benchmark = CrspStockDataset(CrspDatasetConfig.etf_benchmark(
        permno=QQQ_PERMNO, zarr_file_path=str(STORES / "wrds_crsp_qqq_1d.zarr"),
        raw_data_dir_path=str(RAW), reference_dir=str(REFERENCE),
    ))
    ...
    backtester = USEquityCrossectionSelectStockVectorBt(CrossSectionBacktestConfig(
        ...
        rebalance_periods=5, top_n=10, direction="long_only",
