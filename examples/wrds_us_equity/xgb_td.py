"""XGB-TD (pytabkit) pipeline on WRDS CRSP daily data, self-contained.
...
``XGBTDRegressor`` (pytabkit's tuned-default XGBoost; missing
features are filled with 0) -> TopN cross-sectional
...
class Settings:
    ...
    hyperparameters: dict = field(default_factory=lambda: {"n_estimators": 1000, "n_threads": 8})
...
# %% 4. Model
def build_model(s: Settings, p: Paths) -> XGBTDRegressor:
