"""RealMLP (pytabkit) pipeline on WRDS CRSP daily data, self-contained.
...
``RealMLPRegressor`` (pytabkit's tuned-default MLP; it scales its
...
``uv run python examples/wrds_us_equity/realmlp.py`` or step through the
...
class Settings:
    ...
    hyperparameters: dict = field(default_factory=lambda: {"n_epochs": 256, "device": "cpu", "n_threads": 8})
...
# %% 4. Model
def build_model(s: Settings, p: Paths) -> RealMLPRegressor:
